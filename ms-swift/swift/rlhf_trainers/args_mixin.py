...
class GRPOArgumentsMixin(RolloutTrainerArgumentsMixin):
    ...
    Args:
        ...
        tas_enable (bool): Enable TAS-style off-policy replay with a replay buffer. Defaults to False.
        tas_off_policy_batches (int): Number of off-policy mini-batches to mix per generation round. Defaults to 0.
        tas_on_policy_batches (Optional[int]): Target number of on-policy mini-batches per generation round. If set,
            off-policy batches will be capped to keep at least this many on-policy batches. Defaults to None.
        tas_replay_buffer_size (int): Maximum number of replay entries stored. Defaults to 256.
        tas_replay_min_size (int): Minimum buffer size before sampling off-policy batches. Defaults to 16.
        tas_replay_strategy (Literal['fifo', 'priority']): Replay sampling strategy. Defaults to 'priority'.
        tas_priority_reward_weight (float): Weight for reward proxy in replay priority. Defaults to 1.0.
        tas_priority_kl_weight (float): Weight for KL penalty in replay priority. Defaults to 0.1.
        tas_priority_temperature (float): Temperature for priority sampling. Defaults to 1.0.
        tas_store_score_threshold (Optional[float]): If set, only store batches whose replay score meets this threshold.
            Defaults to None.
        tas_hint_key (Optional[str]): Dataset key to use as hint text. If provided, the hint is injected as a message
            before generation. Defaults to None.
        tas_hint_prefix (str): Prefix for injected hint messages. Defaults to 'Hint: '.
        tas_hint_role (Literal['system', 'user']): Role for injected hint messages. Defaults to 'system'.
        tas_lambda (float): TAS parabolic weighting amplitude (λ_TAS). Defaults to 0.3.
        tas_replay_buffer_size (int): Replay buffer size. Defaults to 10000.
        tas_replay_min_size (int): Minimum buffer size before sampling. Defaults to 128.
        tas_replay_easy_ratio (float): Easy tier capacity ratio. Defaults to 0.25.
        tas_replay_medium_ratio (float): Medium tier capacity ratio. Defaults to 0.35.
        tas_replay_hard_ratio (float): Hard tier capacity ratio. Defaults to 0.40.
        tas_vcrs_window (int): Moving average window for VCRS. Defaults to 20.
        tas_off_policy_batches (int): Off-policy mini-batches per generation round. Defaults to 4.
        tas_on_policy_batches (Optional[int]): On-policy mini-batches per generation round. Defaults to 4.
        tas_off_policy_alpha (float): Weight for off-policy loss contribution (α). Defaults to 1.0.
        tas_hint_reward_threshold (Optional[float]): Trigger hint when VCRS for a prompt is below this threshold.
            Defaults to None (disabled).
        tas_hint_kl_threshold (Optional[float]): Trigger hint when KL to reference is below this threshold.
            Defaults to None (disabled).
        tas_hint_zero_patience (int): Trigger hint after this many consecutive zero rewards for a prompt. Defaults to 3.
        tas_hint_always (bool): Always inject hint when available. Defaults to False.
    """
    epsilon: float = 0.2
    ...
    # TAS-style off-policy replay
    tas_enable: bool = False
    tas_off_policy_batches: int = 4
    tas_on_policy_batches: Optional[int] = 4
    tas_off_policy_alpha: float = 1.0
    tas_replay_buffer_size: int = 10000
    tas_replay_min_size: int = 128
    tas_replay_easy_ratio: float = 0.25
    tas_replay_medium_ratio: float = 0.35
    tas_replay_hard_ratio: float = 0.40
    tas_replay_strategy: Literal['fifo', 'priority'] = 'priority'
    tas_priority_reward_weight: float = 1.0
    tas_priority_kl_weight: float = 0.1
    tas_priority_temperature: float = 1.0
    tas_store_score_threshold: Optional[float] = None
    tas_vcrs_window: int = 20

    # Hinting (optional)
    tas_hint_key: Optional[str] = None
    tas_hint_prefix: str = 'Hint: '
    tas_hint_role: Literal['system', 'user'] = 'system'
    tas_hint_reward_threshold: Optional[float] = None
    tas_hint_kl_threshold: Optional[float] = None
    tas_hint_zero_patience: int = 3
    tas_hint_always: bool = False
    tas_lambda: float = 0.3
