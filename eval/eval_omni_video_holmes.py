...
DATA_ROOT = "/eval/Video-Holmes/"
JSON_PATH = f"{DATA_ROOT}/test_Video-Holmes.json"

MODEL_PATH = ""

OUT_PATH = "./exp_results/video_holmes_avatar.jsonl"
CSV_PATH = "./exp_results/video_holmes_avatar.csv"
USE_AUDIO = True
BASE_MODEL = False
NUM_GPUS = 4
...
def reward_fn(sample, model_output):
    try:
        ...
        if output_ans == "":
            letter_match = re.search(r"([A-F])", model_output, re.I)
...
def process_samples_on_gpu(
    ...
    for local_idx, sample in enumerate(data_subset):
        sample_id = sample.get("Question ID", f"sample_{start_idx + local_idx}")

        if sample_id in existing_ids:
            continue

        try:
            video_name = sample["video ID"]
            video_path = os.path.join(DATA_ROOT, "videos", f"{video_name}.mp4")
            ...
            if not final_answer:
                letter_match = re.search(r"([A-F])", reply, re.I)
            ...
            result = {
                ...
                "problem_type": sample.get("Question Type", "N/A"),
...
def calculate_and_save_metrics(out_path, csv_path, model_name="Qwen2.5-Omni"):
    ...
    for result in all_results_data:
        q_type = result.get("problem_type", "N/A")
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as csvfile:
        ...
        row = {q_type: f"{accuracies[q_type]:.4f}" for q_type in question_types}
        row["Overall Accuracy"] = f"{overall_accuracy:.4f}"
    ...
    for q_type in question_types:
        if total_counts[q_type] > 0:
            print(
                f"  {q_type}: {accuracies[q_type]:.2%} ({correct_counts[q_type]}/{total_counts[q_type]})"
            )
        else:
            print(f"  {q_type}: No samples")
...
def main():
    ...
    for idx, sample in enumerate(data):
        sample_id = sample.get("Question ID", f"sample_{idx}")
