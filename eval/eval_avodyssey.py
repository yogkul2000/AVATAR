...
DATA_ROOT = "AV_Odyssey_Bench_LMMs_Eval/"
JSON_PATH = "av_odyssey.json"
MODEL_PATH = ""

OUT_PATH = "./exp_results/grpo_tas_v2.jsonl"
CSV_PATH = "./exp_results/grpo_tas_v2.csv"
USE_AUDIO = True
BASE_MODEL = False
NUM_GPUS = 4

QUESTION_TEMPLATE = (
    "Your task is to accurately answer multiple-choice questions based on the given multimodal content. "
...
def process_samples_on_gpu(
    ...
    for local_idx, sample in enumerate(data_subset):
        sample_id = sample.get("question_id", f"sample_{start_idx + local_idx}")
        ...
        try:
            ...
            if sample.get("image_path") and sample["image_path"]:
                for img_path in sample["image_path"]:
                    full_img_path = os.path.join(DATA_ROOT, img_path)
                    if os.path.exists(full_img_path):
                        content_items.append({"type": "image", "image": full_img_path})
                    else:
                        print(
                            f"[GPU {gpu_id}] Warning: Image file not found: {full_img_path}"
                        )

            if sample.get("video_path") and sample["video_path"]:
                for vid_path in sample["video_path"]:
                    full_vid_path = os.path.join(DATA_ROOT, vid_path)
                    if os.path.exists(full_vid_path):
                        content_items.append({"type": "video", "video": full_vid_path})
                    else:
                        print(
                            f"[GPU {gpu_id}] Warning: Video file not found: {full_vid_path}"
                        )

            audio_files = []
            if sample.get("audio_path") and sample["audio_path"]:
                for aud_path in sample["audio_path"]:
                    full_aud_path = os.path.join(DATA_ROOT, aud_path)
                    if os.path.exists(full_aud_path):
                        content_items.append({"type": "audio", "audio": full_aud_path})
                    else:
                        print(
                            f"[GPU {gpu_id}] Warning: Audio file not found: {full_aud_path}"
                        )
                ...
                conv = [
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": "You are a helpful assistant that answers multiple-choice questions about multimodal content.",
                            }
                        ],
                    },
            ...
            try:
                ...
                try:
                    ...
                    audios, images, videos = audio_files if audio_files else [], [], []
            ...
            result = {
                ...
                "question_type_id": sample.get("question_type_id", "N/A"),
                "data_type": sample.get("data_type", "N/A"),
                "subfield": sample.get("subfield", "N/A"),
...
def calculate_and_save_metrics(out_path, csv_path, model_name="Qwen2.5-Omni"):
    ...
    subfield_correct = defaultdict(int)
    subfield_total = defaultdict(int)
    ...
    for result in all_results_data:
        q_type_id = str(result.get("question_type_id", "N/A"))
        data_type = result.get("data_type", "N/A")
        subfield = result.get("subfield", "N/A")

        total_counts[q_type_id] += 1
        if result.get("correct", False):
            correct_counts[q_type_id] += 1

        type_total[data_type] += 1
        if result.get("correct", False):
            type_correct[data_type] += 1

        subfield_total[subfield] += 1
        if result.get("correct", False):
            subfield_correct[subfield] += 1

    accuracies = {}
    for q_type in total_counts:
        if total_counts[q_type] > 0:
            accuracies[q_type] = correct_counts[q_type] / total_counts[q_type]
        else:
            accuracies[q_type] = 0.0
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as csvfile:
        ...
        writer.writerow(["Results by Question Type ID"])
        writer.writerow(["Question Type ID", "Accuracy", "Correct", "Total"])
        for q_type in sorted(total_counts.keys()):
            accuracy = accuracies[q_type]
            writer.writerow(
                [
                    q_type,
                    f"{accuracy:.4f}",
                    correct_counts[q_type],
                    total_counts[q_type],
                ]
            )
        writer.writerow([])

        writer.writerow(["Results by Data Type"])
        writer.writerow(["Data Type", "Accuracy", "Correct", "Total"])
        for data_type in sorted(type_total.keys()):
            if type_total[data_type] > 0:
                accuracy = type_correct[data_type] / type_total[data_type]
                writer.writerow(
                    [
                        data_type,
                        f"{accuracy:.4f}",
                        type_correct[data_type],
                        type_total[data_type],
                    ]
                )
        writer.writerow([])

        writer.writerow(["Results by Subfield"])
        writer.writerow(["Subfield", "Accuracy", "Correct", "Total"])
        for subfield in sorted(subfield_total.keys()):
            if subfield_total[subfield] > 0:
                accuracy = subfield_correct[subfield] / subfield_total[subfield]
                writer.writerow(
                    [
                        subfield,
                        f"{accuracy:.4f}",
                        subfield_correct[subfield],
                        subfield_total[subfield],
                    ]
                )
    ...
    print("\n📋 Results by Question Type ID:")
    for q_type in sorted(total_counts.keys()):
        if total_counts[q_type] > 0:
            print(
                f"  Type {q_type}: {accuracies[q_type]:.2%} ({correct_counts[q_type]}/{total_counts[q_type]})"
            )
    ...
    for data_type in sorted(type_total.keys()):
        if type_total[data_type] > 0:
            accuracy = type_correct[data_type] / type_total[data_type]
            print(
                f"  {data_type}: {accuracy:.2%} ({type_correct[data_type]}/{type_total[data_type]})"
            )
    ...
    for subfield in sorted(subfield_total.keys()):
        if subfield_total[subfield] > 0:
            accuracy = subfield_correct[subfield] / subfield_total[subfield]
            print(
                f"  {subfield}: {accuracy:.2%} ({subfield_correct[subfield]}/{subfield_total[subfield]})"
            )
